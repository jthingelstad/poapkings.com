...
## RSS feeds
- `src/feeds/roster.xml.njk`: Roster RSS feed (outputs `/feeds/roster.xml`). One `<item>` per member sorted by `date_joined` descending. Excluded from sitemap and collections.
- `dateToRfc2822` filter: Converts an ISO date string to RFC 2822 format for RSS `<pubDate>`.
- `newestDate(items, key)` filter: Returns the most recent date value from an array of objects, used for `<lastBuildDate>`.
