...
- `roster-extra.csv`: Manually-maintained custom member data in CSV format (columns: `tag,name,note,profile_url,address,date_joined`). The `name` column is auto-updated by the sync script for convenience; editable fields are `note`, `profile_url`, `address`, and `date_joined`. Not served.
- `scripts/update-data.py`: Python 3 script that fetches the Clash Royale API and merges with `roster-extra.csv` to produce `src/_data/roster.json`.
...
Custom fields (from `roster-extra.csv`): `note`, `profile_url`, `address`, `date_joined`. The CSV also has a `name` column auto-populated by the sync script.

## Updating the roster
1. Run `python3 scripts/update-data.py` to fetch from Clash Royale API and write `src/_data/roster.json`.
2. Edit `roster-extra.csv` for custom member data (POAP wallets, profile URLs, notes, join dates). The `name` column is auto-updated by the script. Do **not** hand-edit `src/_data/roster.json`.
3. New members not in `roster-extra.csv` get `date_joined` set to today automatically. Members who leave the clan are automatically removed from the CSV.
...
git add src/_data/roster.json roster-extra.csv && git commit -m "Sync roster" && git push
```
...
- `roster-extra.csv` lives at the project root, not in `src/`, since it is not served. It can be opened in any spreadsheet app for easy editing.
