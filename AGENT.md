...
- `src/`: Site source files (templates, static assets, data).
  ...
  - `src/_data/site.json`: Site-wide data (join URL, Tinylytics ID, clan tag, tagline, min trophies, league, status).
...
3. Set `title`, `description`, `navActive` (home/roster/vault/members or omit), and optional flags (`appJs`, `supercellDisclaimer`, `mainId`, `extraScripts`).
